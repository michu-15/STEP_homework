...
'''
    ...
    def find_most_popular_pages(self):
        ...
        for i in range(max_iterations):
            #15％を各ノードに均等に配布
            new_rank = {id :0.15 for id in self.titles}
            #各ノードごとみていく
            for id in self.titles:
                rank = id_rank[id]
                link_targets  = self.links.get(id, [])
                ...
                #リンク先の数だけスコアをわけて、それぞれの行き先にスコア追加
                if link_targets:
                    divide_rank = (random_surfer * rank)/ len(link_targets)
                    for target in link_targets:
                        new_rank[target] += divide_rank
                #リンク先がないと全体にスコア追加
                else:
                    divide_rank = (random_surfer * rank) / num_pages
                    for target in self.titles:
                        new_rank[target] += divide_rank
            #ページランクの合計の計算
            total_rank  = sum(new_rank.values())
            print(f"Iteration {i + 1}: total rank sum = {total_rank:.6f}")
