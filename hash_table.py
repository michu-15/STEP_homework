...
def calculate_hash(key):
    ...
    hash = 0
    for i in key:
        hash = hash * 10 + ord(i)    #Update Hash
    return hash
...
class HashTable:
    ...
    def delete(self, key):
        ...
        while item:
            if item.key == key:
                if before_item != None:
                    before_item.next = item.next #削除したい前後のノードをつなぐ　Connect nodes before and after want to delete
                else: #when item is first node 探しているのが先頭のノードの時
                    self.buckets[bucket_index] = before_item.next
                self.item_count -= 1
                return  True
            before_item = item #次のノードに移動
            item = item.next
        return False


'''       
        #Count node index 何番目のノードかを確認するための関数
        def get_nth_item(start, n):
            current = start
            index = 0
            while current:
                if index == n:
...
'''
    ...
    def size(self):
        if self.item_count > self.bucket_size * 0.7:
            self.re_hash(self.bucket_size  * 2)
    ...
    #再ハッシュ用の関数を作る　Creating a rehash function
    def re_hash(self, item,  new_size):
